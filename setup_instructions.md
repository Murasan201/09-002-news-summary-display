...
- SSD1306 OLEDディスプレイモジュール（I²C接続、128×64ピクセル、3.3V動作）
- ジャンパーワイヤ（オス-オス、4本）
- ブレッドボード（汎用サイズ）
- 電源（公式5V/3A電源アダプタ推奨）
...
### 2.1 SSD1306 OLED (3.3V) の接続

このプロジェクトでは、3.3V動作のSSD1306 OLEDモジュールを使用します。レベル変換器は不要です。
